...
class Adasyn(object):
    ...
    def fit(self, X, y):
        ...
        if len(unique_classes) == 1:
            ...
            if not self.binary:
                raise ValueError(
                    'Set binary=True for binary classification problems')
        else:
            if self.binary:
                raise ValueError(
                    'Set binary=False (default) for multiclass problems')
        ...
        for cl in unique_classes:
            ...
            if imb_degree > self.imb_threshold:
                raise ValueError(
                    'Imbalance threshold not satisfied, try reducing imb_threshold parameter')
            else:
